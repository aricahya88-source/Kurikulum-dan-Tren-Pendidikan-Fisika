...
export default function AdminActivities(){
  ...
  return <AuthGate adminOnly><AppShell title="Kelola Tugas">
    <div className="admin-split"><GlassCard className="admin-list"><div className="row between"><div><span className="eyebrow">AKTIVITAS</span><h3>Tugas, Proyek, & Partisipasi</h3></div><button className="icon-button" onClick={()=>setA({...empty})}><Plus/></button></div><div className="scroll-list">{rows.map(x=><button key={x.activity_id} className={a.activity_id===x.activity_id?'select-row active':'select-row'} onClick={()=>setA(x)}><strong>{x.title}</strong><small>{x.week_id} • {x.type}</small></button>)}</div></GlassCard>
    <GlassCard>
      <div className="form-grid two">
        <label className="field"><span>Minggu</span><select value={a.week_id} onChange={e=>setA({...a,week_id:e.target.value})}>{Array.from({length:16},(_,i)=><option key={i} value={`W${String(i+1).padStart(2,'0')}`}>Minggu {i+1}</option>)}</select></label>
        <label className="field"><span>Jenis</span><select value={a.type} onChange={e=>setA({...a,type:e.target.value})}><option value="assignment">Tugas</option><option value="checkpoint">Checkpoint</option><option value="reflection">Refleksi</option><option value="peer_review">Peer Review</option><option value="test">Testing</option><option value="presentation">Presentasi</option><option value="project">Proyek</option><option value="participation">Partisipasi</option></select></label>
      ...
      {a.type==='project'&&<label className="field"><span>Project code</span><input value={a.project_code||''} onChange={e=>setA({...a,project_code:e.target.value.toUpperCase()})} placeholder="STUDENT_LED / UTS_BLUEPRINT / UAS_CURRICULUM"/></label>}
      </div>
