...
import { ArrowLeft,ClipboardCheck,FolderKanban,ArrowRight,Target,UsersRound,CheckCircle2 } from 'lucide-react';
...
export default function WeekPage(){
 ...
 const href=(a:Activity)=>a.type==='project'&&a.project_code?`/projects/${a.project_code}`:`/tasks/${a.activity_id}`;
 ...
 return <AuthGate><AppShell title={`Pertemuan ${weekNo}`}><Link href="/weeks" className="button soft compact"><ArrowLeft/>Kembali</Link>{error&&<div className="error-box">{error}</div>}{!d&&!error?<div className="screen-center small"><div className="spinner"/>Memuat...</div>:null}{d&&<div className="stack">
  ...
  {plan&&<div className="two-column"><div className="glass-card"><div className="row gap"><div className="icon-bubble teal"><Target/></div><div><span className="eyebrow">CAPAIAN</span><h3>{plan.cpmk.join(' • ')}</h3></div></div><ul>{plan.objectives.map((x,i)=><li key={i}>{x}</li>)}</ul></div><div className="glass-card"><div className="row gap"><div className="icon-bubble amber"><UsersRound/></div><div><span className="eyebrow">AKTIVITAS KELAS</span><h3>{plan.mode}</h3></div></div><ul>{plan.during.map((x,i)=><li key={i}>{x}</li>)}</ul></div></div>}
  ...
  {(plan?.activityHref||d.activities.length)?<div className="activity-grid">
    ...
    {d.activities.map(a=><Link key={a.activity_id} href={href(a)} className="glass-card activity-card"><div className="icon-bubble coral">{a.type==='project'?<FolderKanban/>:<ClipboardCheck/>}</div><div className="grow"><span className="eyebrow">{a.type.toUpperCase()}</span><h3>{a.title}</h3></div><ArrowRight/></Link>)}
  </div>:<div className="notice">Tidak ada submission pada pertemuan ini. Fokus pada aktivitas pembelajaran kelas.</div>}
