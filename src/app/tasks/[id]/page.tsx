...
import ArticleAnalysisForm from '@/components/ArticleAnalysisForm';
import ProgramAnalysisForm from '@/components/ProgramAnalysisForm';
...
export default function TaskPage({params}:{params:Promise<{id:string}>}){
  ...
  return <AuthGate><AppShell title="Tugas">
    ...
    {!d?<div className="screen-center small"><div className="spinner"/>Memuat tugas...</div>:<div className="stack">
      ...
      {id==='ARTICLE_ANALYSIS'?<ArticleAnalysisForm activityId={id} latestHtml={d.latest?.content_html} onSubmitted={load}/>:id==='PROGRAM_ANALYSIS'?<ProgramAnalysisForm activityId={id} latestHtml={d.latest?.content_html} onSubmitted={load}/>:<GlassCard><span className="eyebrow">{d.latest?'KIRIM REVISI':'KUMPULKAN'}</span><h3>Submission</h3><RichTextEditor value={content} onChange={setContent} minHeight={160}/>
