...
export default function ProjectFinalReport({code}:{code:string}){
  ...
  return <div className="stack project-final-section">
    <GlassCard className="final-report-hero"><div className="row between wrap gap"><div className="row gap"><div className="icon-bubble coral"><FileCheck2/></div><div><span className="eyebrow">TAHAP FINAL</span><h2>{code==='STUDENT_LED'?'Realisasi & Refleksi Sesi':code==='UTS_BLUEPRINT'?'Final Curriculum Blueprint':'Final Training Curriculum'}</h2><p className="muted">{code==='STUDENT_LED'?'Dokumentasikan realisasi strategi, keterlibatan kelas, hasil pengecekan pemahaman, dan refleksi perbaikan.':code==='UTS_BLUEPRINT'?'Kirim Training Needs Analysis, Curriculum Blueprint/Map, ringkasan revisi setelah clinic, serta bukti oral defense.':'Kirim dokumen kurikulum pelatihan final, Curriculum Map/Canvas, Implementation Plan, ringkasan alignment audit, serta bukti final pitch dan oral defense.'}</p></div></div>{d?.is_group?<span className="badge"><UsersRound/>{d.group?.name||'Kelompok'}</span>:<span className="badge">Individu</span>}</div></GlassCard>
